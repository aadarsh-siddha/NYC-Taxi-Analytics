CREATE TABLE IF NOT EXISTS nyc_taxi_analytics.bronze.yellow_trips (
...
COPY INTO nyc_taxi_analytics.bronze.yellow_trips
FROM (
  SELECT
    $1:VendorID::NUMBER,
    $1:RatecodeID::NUMBER,
    ...
    CURRENT_TIMESTAMP()
  FROM @nyc_taxi_analytics.bronze.my_s3_stage/yellow/{{ macros.ds_format(macros.ds_add(ds, -60), '%Y-%m-%d', '%Y') }}/{{ macros.ds_format(macros.ds_add(ds, -60), '%Y-%m-%d', '%m') }}/
)
